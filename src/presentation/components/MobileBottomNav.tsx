...
export const MobileBottomNav: React.FC<MobileBottomNavProps> = ({
  ...
  const tabs = [
    ...(isOwner ? [{ id: 'dashboard' as ActiveNavTab, label: 'Dash', icon: LayoutDashboard }] : []),
  ...
  return (
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-slate-950/95 backdrop-blur-lg border-t border-slate-800/80 px-2 py-1.5 flex items-center justify-around safe-bottom shadow-2xl">
      {tabs.map((tab) => {
        const Icon = tab.icon;
